...
const DISMISSED_KEY = 'pfmanager:install-dismissed'
...
/**
 * Offers "add to home screen", so the app opens in one tap.
 *
...
export function InstallPrompt() {
  ...
  // Both decided once, at mount: install state and the stored dismissal
  // cannot change mid-session, so they are initial state, not an effect.
  const [dismissed, setDismissed] = useState(() => isStandalone() || wasDismissedBefore())
  const [showIosHint, setShowIosHint] = useState(() => !dismissed && isIos())
  ...
  return (
    <div className="mb-3 flex items-start gap-3 rounded-lg border border-accent/40 bg-surface p-3">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold text-fg">Add pfmanager to your home screen</p>

        {deferred ? (
          <p className="mt-0.5 text-xs text-fg-muted">
            Opens in one tap and works with no connection.
          </p>
        ) : (
          <>
            <p className="mt-0.5 text-xs text-fg-muted">
